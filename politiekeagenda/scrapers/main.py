 ...
 * Properly handle server errors on scraping (possibly halt scraping and retry later)
...
import bof.politiekeagenda.config as config

from bof.politiekeagenda.helpers import *
...
def update_or_create_assembly(assembly_dict):
    ...
    try:
        assembly = session.query(models.Assembly).filter(models.Assembly.url==assembly_dict["url"]).one()
...
def scrape(bypass_cache=False):
    ...
    def scrape_senate_plenary():
        ...
        try:
            document = retrieve_if_not_exists(index_url, bypass_cache=bypass_cache)
        ...
        for element in document.xpath("//a[contains(@href, '/plenaire_vergadering/')]"):
            ...
            try:
                document = retrieve_if_not_exists(assembly_detail_url, bypass_cache=bypass_cache)
            except urllib2.HTTPError:
    ...
    def scrape_senate_committee():
        ...
        try:
            document = retrieve_if_not_exists(committees_index_url, bypass_cache=bypass_cache)
        except urllib2.HTTPError:
        ...
        for element in document.xpath(".//a[contains(@href, '/commissies/')]"):
            ...
            try:
                document = retrieve_if_not_exists(committee_page_url, bypass_cache=bypass_cache)
            except urllib2.HTTPError:
            ...
            document = retrieve_if_not_exists(committee_activities_url, bypass_cache=bypass_cache)
    ...
    def scrape_house_plenary():
        ...
        document = retrieve_if_not_exists(url, bypass_cache=bypass_cache)
    ...
    def mark_missing():
        ...
        assemblies = (session.query(models.Assembly)
            .filter(or_(models.Assembly.missing==0, models.Assembly.missing==None))
            .all())
